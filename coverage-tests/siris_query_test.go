...
func createApp_Query() *siris.Application {
	...
	app.Get("/read", func(c context.Context) {
		...
		if err2 != nil {
			c.StatusCode(siris.StatusNotAcceptable)
			return
		}
...
func TestSiris_Query(t *testing.T) {
	...
	b1.Equal("category=c1&category=c2&page=2&page_size=50&search=my+search")

	b2 := e.GET("/read").
		WithQueryString("category=c3&category=c4&page=5&page_size=500&search=my+siris").
		Expect().
		Status(http.StatusOK).Body()
	b2.Equal("category=c3&category=c4&page=5&page_size=500&search=my+siris")
}
