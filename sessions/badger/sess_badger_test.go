...
func TestSessionBader(t *testing.T) {
	config := `{"cookieName":"gosessionid","gclifetime":600,"maxLifetime": 300,"domain":"example.com","enableSetCookie":true,"EnableSidInHTTPHeader":true,"SessionNameInHTTPHeader":"Gosessionid","providerConfig":""}`
	conf := new(sessions.ManagerConfig)
	if err := json.Unmarshal([]byte(config), conf); err != nil {
		t.Fatal("json decode error", err)
	}
	globalSessions, _ := sessions.NewManager("badger", conf)
...
func TestSessionBadgerPanic_Register(t *testing.T) {
	defer func() {
		assert.NotNil(t, recover())
	}()
	sessions.Register("badger", nil)
}

func TestSessionBadgerPanic_RegisterDup(t *testing.T) {
	defer func() {
		assert.NotNil(t, recover())
	}()
...
func TestSessionBadgerPanic_Manager(t *testing.T) {
	defer func() {
		assert.NotNil(t, recover())
	}()

	config := `{"cookieName":"gosessionid","gclifetime":600,"maxLifetime": 300,"domain":"example.com","enableSetCookie":true,"EnableSidInHTTPHeader":true,"SessionNameInHTTPHeader":"","providerConfig":""}`
	conf := new(sessions.ManagerConfig)
	if err := json.Unmarshal([]byte(config), conf); err != nil {
		t.Fatal("json decode error", err)
	}
	_, _ = sessions.NewManager("badger", conf)
}

func TestSessionBadgerPanic_Manager2(t *testing.T) {
	defer func() {
		assert.NotNil(t, recover())
	}()
