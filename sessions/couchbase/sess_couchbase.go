...
// Usage:
// import(
//   _ "github.com/go-siris/siris/sessions/couchbase"
//   "github.com/go-siris/siris/sessions"
// )
//
//	func init() {
//		globalSessions, _ = sessions.NewManager("couchbase", ``{"cookieName":"gosessionid","gclifetime":3600,"ProviderConfig":"http://host:port/, Pool, Bucket"}``)
...
func (cp *Provider) SessionRead(sid string) (sessions.Store, error) {
	cp.b = cp.getBucket()
...
func (cp *Provider) SessionRegenerate(oldsid, sid string) (sessions.Store, error) {
	cp.b = cp.getBucket()
...
func init() {
	sessions.Register("couchbase", couchbpder)
}
