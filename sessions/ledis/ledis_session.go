...
func (lp *Provider) SessionRead(sid string) (sessions.Store, error) {
	var (
...
func (lp *Provider) SessionRegenerate(oldsid, sid string) (sessions.Store, error) {
	count, _ := c.Exists([]byte(sid))
	if count == 0 {
...
func init() {
	sessions.Register("ledis", ledispder)
}
